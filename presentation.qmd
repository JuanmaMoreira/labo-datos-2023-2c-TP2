...
En principio los datos parecen estar bastante dispersos, sin embargo, en este último gráfico puede observarse una separación de los datos en dos nubes (una encima de otra). Veamos que pasa si separamos los puntos por color en base a si son de un día laborable o no:
...
El modelo que armamos es de la forma y = b0 + b2⋅tavg cuando es día no laborable, y cuando es día laborable es de la forma y = b0+b1 + b2⋅tav. Por lo tanto vemos que ambas rectas van a tener la misma pendiente y van a diferir en el valor de b1.
...
En esta segunda parte realizamos un clasificador de noticias en "fake news" y en "reales". Trabajamos con las variables predictoras:
...
fake_news %>%
  ...
  geom_col()+
  labs(title = 'Cantidad de noticias que contienen signos de exclamación', x= 'No contiene / Contiene', y='Cantidad')+
